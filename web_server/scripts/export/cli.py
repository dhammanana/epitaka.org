...
Examples:
  ...
  %(prog)s --book Dhp --lang vi --output-dir ./out   # Vietnamese, custom output dir
    ...
    parser.add_argument(
        ...
        type=str,
        default="en",
        help="Translation language code (e.g. en, si, th; default: en)",
    )
    parser.add_argument(
        "--script",
        "-s",
    ...
    parser.add_argument(
        "--no-cover", action="store_true", help="Skip cover image generation"
    )
    parser.add_argument(
        "--mula-only",
        action="store_true",
        default=False,
        help="Export only Mūla (original) books",
    )
    parser.add_argument(
        "--no-overwrite",
        "-no",
    ...
    for fmt in formats:
        if fmt not in valid_formats:
            parser.error(
                f"Invalid format: {fmt}. Choose from: {', '.join(sorted(valid_formats))}"
            )
    ...
    for idx, book in enumerate(books, start=1):
        ...
        print(f"\n{'=' * 60}")
        print(
            f"📖 [{idx}/{total_books}] {book.book_name} ({book.book_id})"
        ...
        for fmt in formats:
            ...
            if (
                ...
                print(f"  ⏭️  {fmt.upper():<6} → skipped (exists): {rel_path}")
                skipped += 1
                continue
        ...
        print(f"  ⏱️  [{idx}/{total_books}] done in {book_elapsed:.1f}s")

    # ── Summary ───────────────────────────────────────────────────────
    elapsed = time.time() - start_time
    print(f"\n{'=' * 60}")
    print(f"✨ Done! {success} files exported, {failed} failed, {skipped} skipped")
